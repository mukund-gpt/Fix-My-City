...
const StatCard = ({ title, value, color, icon: Icon }) => (
  <Card sx={{ 
    ...
    <CardContent>
      ...
      <Typography variant="h4" component="div" fontWeight="bold" mt={1} color={color}>
        {value}
      </Typography>
    </CardContent>
  </Card>
...
const ViewAnalytics = () => {
  ...
  const statusColors = [theme.palette.error.main, theme.palette.warning.main, theme.palette.success.main];
  const urgencyColors = [theme.palette.primary.light, theme.palette.warning.dark, theme.palette.error.dark];
  ...
  const resolvedPercentage = totalComplaints > 0 ? ((data.resolved / totalComplaints) * 100).toFixed(1) : 0;

  return (
    <Box className="p-6 bg-gray-50 min-h-screen">
      <Typography variant="h4" className="mb-8 font-extrabold text-gray-800 border-b pb-2">
      ...
      <Grid container spacing={4} className="mb-8">
        <Grid item xs={12} sm={4}>
          <StatCard title="Total Complaints" value={totalComplaints} color={theme.palette.primary.main} />
        </Grid>
        <Grid item xs={12} sm={4}>
          <StatCard title="Resolved Rate" value={`${resolvedPercentage}%`} color={theme.palette.success.main} />
        </Grid>
        <Grid item xs={12} sm={4}>
          <StatCard title="Open Complaints" value={data.open} color={theme.palette.error.main} />
        </Grid>
      </Grid>
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={6}>
          <Card className="shadow-lg rounded-xl h-96"> {/* Increased height for better chart aspect ratio */}
            <CardContent>
