...
complaintSchema.methods.escalate = async function (
  ...
  if (Notification) {
    ...
    console.log(
      `[Escalation] Notified ${staffIdsToNotify.size} staff members for Complaint ${this._id}.`,
    );
  } else {
...
complaintSchema.statics.escalateOverdueComplaints = async function () {
  ...
  for (const complaint of complaints) {
    ...
    if (
      ...
      const ttaDeadline = new Date(
        complaint.createdAt.getTime() + urgencyConfig.TTA * 60 * 60 * 1000,
      );
    ...
    if (targetEscalationLevel > complaint.escalationLevel) {
      ...
      if (!targetStaffId) {
        console.warn(
          `Could not find a staff member for Level ${targetEscalationLevel} escalation on complaint ${complaint._id}. Skipping.`,
        );
        continue;
      }
