...
import { Box, Button, CircularProgress, Divider, Drawer, IconButton, TextField, Typography } from "@mui/material"; // Imported Drawer and TextField
import { useEffect, useState } from "react";
import toast from "react-hot-toast";
import { useSelector } from "react-redux";
import { useParams } from "react-router-dom";
...
// --- Mock Components (In a real app, move these to separate files) ---
...
const CommentDisplay = ({ comment }) => {
    ...
    return (
        <Box className="p-3 border rounded-lg shadow-sm bg-blue-50">
            ...
            <Typography variant="body2" className="text-gray-800 whitespace-pre-wrap mb-2">
                {comment.commentText}
            </Typography>
                    ...
                    {(userRole === "admin" || userRole === "staff") && (
                        <Box className="lg:col-span-1"> 
                            <Box className="bg-white p-6 rounded-xl shadow-2xl sticky top-4">
                                ...
                                <Typography variant="h6" component="h2" className="font-bold text-indigo-600 mb-4">
                                    {userRole === "admin" ? "Admin Actions" : "Resolution Log & Actions"}
                                </Typography>
                                <Divider className="mb-4" />
                                ...
                                {userRole === "staff" && (
                                    <Box className="mb-6">
                                        ...
                                        <Box className="max-h-96 overflow-y-auto space-y-3 p-1">
                                            ...
                                            {complaint?.commentList && complaint.commentList.length > 0 ? (
                                                complaint.commentList.map((comment) => (
                                                    <CommentDisplay key={comment._id} comment={comment} /> 
                                                ))
                                            ) : (
                                                <p className="text-sm text-gray-500">No resolution comments yet.</p>
                                            )}
                                        </Box>
                                    </Box>
                                )}
