...
export default function Reports() {
  ...
  const MetricCard = ({ title, value, unit, icon: Icon, color }) => (
    <div className={`p-5 rounded-2xl shadow-xl transition duration-300 transform hover:shadow-2xl ${color} text-white`}>
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 border-b pb-4">
          ...
          <p className="text-gray-500 mt-1">
            Generate key performance indicators and download reports for detailed analysis.
          </p>
        </header>

        {/* --- Date Filter and Action Bar (Card) --- */}
        <div className="bg-white p-6 rounded-2xl shadow-2xl mb-8 border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Select Reporting Period</h3>
          <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-end">
            ...
            <button
              ...
              disabled={isLoading}
              className={`w-full md:w-auto px-6 py-3 rounded-xl font-semibold transition duration-200 transform hover:scale-[1.02] active:scale-100 ${isLoading ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-md hover:bg-blue-700'}`}
            >
                    ...
                    return (
                      <li key={category} className="p-4 bg-gray-50 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center transition duration-150 hover:bg-gray-100 border-l-4 border-blue-500">
                        ...
                        <span className="text-right">
                          <span className="font-bold text-lg text-gray-800 mr-2">{count}</span>
        ...
        {!analytics && !isLoading && (
          <div className="mt-12 text-center p-10 bg-white rounded-2xl shadow-lg border border-gray-100">
            ...
            <p className="text-gray-600">
              Please select a date range and click "View Analytics" to load the report.
            </p>
          </div>
        )}
      </div>
