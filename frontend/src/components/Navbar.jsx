    ...
    <AppBar position="sticky" 
      ...
      <Toolbar className="max-w-7xl mx-auto w-full flex justify-between">
            ...
            <li key={tab.name}>
              {tab.options ? (
                ...
                >
                  ...
                  <span>{tab.name}</span>
                  <ExpandMoreIcon fontSize="small" className="ml-1 text-gray-300" />
                </button>
              ) : (
                <Link
                  to={tab.path}
        ...
        {!user ? (
          ...
          <div className="hidden md:flex items-center">
            <IconButton onClick={handleProfileOpen} className="p-1 hover:scale-110 transition-transform">
              <Avatar src={user?.avatar || ""} alt={user?.name || "User"} className="bg-yellow-400 text-black" />
              <Typography className="ml-5 text-white">{user?.name || "User"}</Typography>
            </IconButton>
          </div>
        )}
      ...
      {currentTab?.options && (
        <Menu
          ...
          PaperProps={{ className: "bg-gray-800 rounded-lg shadow-lg shadow-yellow-400/20" }}
        >
      ...
      {user && (
        <Menu
          ...
          TransitionComponent={Fade}
          PaperProps={{ className: "bg-gray-800 text-white rounded-lg shadow-lg" }}
        >
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          ...
          <IconButton onClick={() => setDrawerOpen(false)} className="text-white">
            <CloseIcon />
          </IconButton>
        </div>
        <List>
