...
export const getReports = async (req, res) => {
  try {
    ...
    const avgResolutionTimeInMillis =
      totalResolutionTimeInMillis / resolvedComplaints.length || 0;
    const avgResolutionTimeInHours =
      avgResolutionTimeInMillis / (1000 * 60 * 60);
    ...
    if (format === "csv") {
      ...
      const processedData = complaints.map((c) => ({
        ...c,
        assignedStaff:
          c.assignedTo?.map((staff) => staff.name).join(", ") || "N/A",
        department:
          c.assignedTo
        ...
        comments:
          c.commentList
            ?.map(
              (com) =>
                `${com.author?.name}: ${com?.commentText}${com?.imageUrl?.length ? " [Image attached]" : ""}`,
            )
            .join(" | ") || "No comments",
      }));
    ...
    if (format === "pdf") {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="complaints-report.pdf"',
      );
      doc.pipe(res);
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("Complaints Report", { align: "center" });
      ...
      doc.text(
        `Average Resolution Time: ${avgResolutionTimeInHours.toFixed(2)} hours`,
      );
      doc.text(
        `SLA Compliance (Target ${SLA_HOURS}h): ${slaCompliance.toFixed(1)}%`,
      );
      doc.moveDown();
      ...
      doc
        .fontSize(16)
        .font("Helvetica-Bold")
        .text("Detailed Complaints List", { underline: true });
      doc.moveDown();
