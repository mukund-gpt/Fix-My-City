...
const CircularProgress = () => <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>;
...
const ConfigureSLA = ({ onBack }) => {
    ...
    const handleChange = useCallback((urgency, type, value) => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                setMessage({ type: 'error', text: 'Save operation failed on the server.' });
            }
        } catch (error) {
            console.error("Save error:", error);
            setMessage({ type: 'error', text: 'An unexpected error occurred during saving.' });
        } finally {
            setSaving(false);
    ...
    const renderSLASection = (urgency) => {
        ...
        let colorClass = '';
        switch (urgency) {
            case 'HIGH': colorClass = 'text-red-500 border-red-500'; break;
            case 'MEDIUM': colorClass = 'text-yellow-500 border-yellow-500'; break;
            case 'LOW': colorClass = 'text-green-500 border-green-500'; break;
            default: colorClass = 'text-gray-500 border-gray-500'; break;
        }

        return (
            <div className={`p-6 rounded-xl shadow-2xl transition duration-300 bg-white hover:shadow-xl border-t-8 ${colorClass.replace('text-', 'border-t-')}`}>
        ...
        <div className="p-6 bg-gray-900 rounded-xl text-white shadow-2xl font-sans">
            ...
            {!loading && (
                <form onSubmit={handleSave}>
                    ...
                    {/* Message Area */}
                    {message && (
                        <div className={`p-4 rounded-xl mb-6 flex items-center text-sm font-semibold ${
